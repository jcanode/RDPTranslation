...
## load data
# Note: deleted /data from both paths for Emilee's computer
spanish_data_path = os.path.join(os.path.curdir,"C:/Users/User/Documents/GitHub/RDPTranslation/europarl-v7.es-en.es")
english_data_path = os.path.join(os.path.curdir,"C:/Users/User/Documents/GitHub/RDPTranslation/europarl-v7.es-en.en")
...
start_time = time.time()
def create_dataset(span_path, eng_path):
  span_lines = io.open(span_path, encoding='UTF-8').read().strip().split('\n')
  eng_lines = io.open(eng_path, encoding='UTF-8').read().strip().split('\n')
  return span_lines, eng_lines
spanish_data, english_data = create_dataset(spanish_data_path, english_data_path)
...
# Changed path for Emilee's computer
# english_config_path = os.path.join("./tokenizerConfig/english_config.json")
english_config_path = os.path.join("C:/Users/User/Documents/GitHub/RDPTranslation/tokenizerConfig/en_config.json")
## Uncomment these lines for first time running the program to configure tokenizer (helps speed up run time)
...
es_tokenizer = tf.keras.preprocessing.text.Tokenizer(filters='')

## Uncomment these lines for first time running the program to configure tokenizer (helps speed up run time)
es_tokenizer.fit_on_texts(spanish_data)
spanish_config = es_tokenizer.to_json()

# german_config_path = os.path.join("./tokenizerConfig/german_config.json")
# Emilee's Path
spanish_config_path = os.path.join("C:/Users/User/Documents/GitHub/RDPTranslation/tokenizerConfig/es_config.json")

## Uncomment these lines for first time running the program to configure tokenizer (helps speed up run time)
io.open(spanish_config_path,'w').write(spanish_config)
print("Wrote spanish config to ", spanish_config_path)
...
data_es = es_tokenizer.texts_to_sequences(spanish_data)
data_es = tf.keras.preprocessing.sequence.pad_sequences(data_es,padding='post',maxlen=668) ##pad german dataset to same length as english
...
X_train,  X_test, Y_train, Y_test = train_test_split(data_en,data_es,test_size=0.8)
...
seq2seq.save(filepath="./models/seq2seq_en_es")
